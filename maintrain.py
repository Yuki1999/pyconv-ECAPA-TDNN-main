...
# 创建随机数据集
def create_random_batch(batch_size, num_features):
    X = np.random.rand(batch_size, num_features).astype(np.float32)
...
# 生成随机数据集
batch_size = 32
num_features = 16000  # 假设每个样本为1秒的音频，采样率为16kHz
X, y = create_random_batch(batch_size, num_features)
...
optimizer.zero_grad()
outputs = model(X_tensor, aug=False)
...
if torch.cuda.is_available():
    ...
    print("GPU is not available. Training is done on CPU.")
# 打印模型参数
